...
T parallel_accumulate(Iterator begin, Iterator end, T init){
    ...
    auto result =0;
    std::thread other(accumulate_block<Iterator, T>, begin, middle, 0, std::ref(result));
...
int main(){
    ...
    std::cout << parallel_accumulate(numbers.begin(), numbers.end(), 0) << std::endl;

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> d(1, 10);
    std::vector<int> test_nums;
    std::generate_n(std::back_inserter(test_nums), 10'000'00, [&gen, &d](){return d(gen);});

    std::cout << "Parallel" <<std::endl;
    {
        Timer t;
        auto result = future_accumulate(test_nums.begin(), test_nums.end(), 0);

    }
    std::cout << "Sequential" <<std::endl;
    {
        Timer t;
        auto result = std::accumulate(test_nums.begin(), test_nums.end(), 0);
